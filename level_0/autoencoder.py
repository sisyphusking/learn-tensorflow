...
class Autoencoder:
        ...
        with tf.Session() as sess:
            ...
            for i in range(self.epoch):
                ...
                if i % 10 == 0:
                    ...
                    self.saver.save(sess, '../data/autoencoder/model.ckpt')
            self.saver.save(sess, '../data/autoencoder/model.ckpt')

    def test(self, data):
        with tf.Session() as sess:
            self.saver.restore(sess, '../data/autoencoder/model.ckpt')
            # 第一个参数是encoded的返回值，第二个参数是decoded返回值
    ...
    def get_params(self):
        with tf.Session() as sess:
            self.saver.restore(sess, '../data/autoencoder/model.ckpt')
            weights, biases = sess.run([self.weights1, self.biases1])  # 这里的参数应该是编码器的参数。取第一层的weights
